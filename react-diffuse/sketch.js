...
// // Blue/Pink (colourful) colour parameters
// // Background colour
// var myred = 0;
// var mygreen = 100;
// var myblue = 160;
...
function draw(){
	...
	for (var x = 0; x < width; x++){
		for (var y = 0; y < height; y++){
			...
			pixels[pixel + 0] = myred + floor(red_mult*(b)); // red
			pixels[pixel + 1] = mygreen + floor(red_mult*(b)); // green
			pixels[pixel + 2] = myblue + floor(red_mult*(b)); // blue
	...
	if (start){
		...
		textSize(18);
		text("tip: if things are slow, try reducing your browser window size", width/2, height*19/20);
	} else if (!play){
		// Pause screen
		textStyle(ITALIC);
		textAlign(CENTER);
